...
for line in inscriptions:
    ...
    if cur_doss in dossards_inscrits:
        ...
        exit(1)
    coureurs[cur_doss]={"nom":cur_l[0].strip(),
...
while nb_rfids_vus < nb_rfids_inscrits:
    ...
    with open(SYNTHESE_TEMPO, "w") as synthese_tmp_f:
        ...
        synthese_tmp_f.write("NOM\tPRENOM\tSEXE\tNAISSANCE\tNUMERO\tTEMPS\tCOURSE\tDISTANCE\tVILLE\tEMAIL\tCLUB\tLICENCE\n")
                ...
                for line in file:
                    ...
                    # print("not seen "+rfid_cand)
                        ...
                        if rfid_cand in rfids_inscrits:
                            ...
                            delta = tf - t0
                            synthese_tmp_f.write(f"{cur_cour['nom']}\t{cur_cour['prénom']}\t{cur_cour['genre']}\t{cur_cour['date_de_naissance']}\t{cur_dos}\t{delta}\t{cur_cour['cross']}\t{cur_cour['km']}\t{cur_cour['village']}\t{cur_cour['email']}\t{cur_cour['club']}\t{cur_cour['license']}\n")
                            nb_rfids_vus = nb_rfids_vus + 1
                            if cur_dos not in cur_doss_vus:
                                cur_dossards_vus=cur_dossards_vus+"["+str(cur_dos)+"]"
